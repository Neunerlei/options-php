...
class OptionsTest extends TestCase {
	...
	public function testDefaultValue() {
		...
		$this->assertEquals(["foo" => 123123], Options::make([], ["foo" => function ($k, $options, $definition, $path) {
		...
		try {
			...
			$this->assertStringContainsString("Definition error at: \"foo\"; An empty array was given as definition.", $e->getMessage());
		}
	...
	public function testSingleTypeValidation() {
		...
		$sets = [
			...
			[
				"types"                 => ["number"],
				"data"                  => [123, 546345, 12123, 56456, 1230, 0, 123.23, 546345.12325,
											12123.123, 56456.1, 1230.45, 0,
				],
			...
			[
				"types"                 => ["numeric"],
				"data"                  => [123, 546345, 12123, 56456, 1230, 0, 123.23, 546345.12325,
											12123.123, 56456.1, 1230.45, 0, "123", "12.123",
				],
		...
		foreach ($realSets as $type => $set) {
			...
			foreach ($realSets as $invalidType => $invalidSet) {
				if ($invalidType === $type) continue;
				if (!empty($set["ignoredForInvalidData"]) && in_array($invalidType, $set["ignoredForInvalidData"])) continue;
	...
	public function testMultiTypeValidation() {
		$this->assertEquals(["foo" => "bar"], Options::make(["foo" => "bar"], ["foo" => ["type" => ["string", "bool"]]]));
		$this->assertEquals(["foo" => TRUE], Options::make(["foo" => TRUE], ["foo" => ["type" => ["string", "bool"]]]));
		$this->assertEquals(["foo" => FALSE], Options::make(["foo" => FALSE], ["foo" => ["type" => ["string", "bool"]]]));
		$a = new DummyExtendedClassA();
		$this->assertEquals(["foo" => $a], Options::make(["foo" => $a], ["foo" => ["type" => [DummyClassA::class, "bool"]]]));
		...
		try {
			...
			$this->assertInstanceOf(OptionValidationException::class, $e);
			$this->assertStringContainsString("-Invalid value type at: \"foo\" given; Allowed types: \"string\" or \"bool\"", $e->getMessage());
		}
		try {
			...
			$this->assertInstanceOf(OptionValidationException::class, $e);
			$this->assertStringContainsString("-Invalid value type at: \"foo\" given; Allowed types: \"null\" or \"bool\"", $e->getMessage());
		}
	}
	...
	public function testDefaultTypeValidation() {
		...
		try {
			...
			$this->assertInstanceOf(OptionValidationException::class, $e);
			$this->assertStringContainsString("-Invalid value type at: \"foo\" given; Allowed types: \"string\"", $e->getMessage());
		}
	}
	...
	public function testValidator() {
		...
		try {
			...
			$this->assertInstanceOf(OptionValidationException::class, $e);
			$this->assertStringContainsString("-Validation failed at: \"foo\" - Custom error message", $e->getMessage());
		}
	}
	...
	public function testValueValidation() {
		...
		// Positive validation
		$this->assertEquals(["foo" => TRUE], Options::make(["foo" => TRUE], ["foo" => ["values" => ["true", TRUE, 1]]]));
		...
		try {
			...
			$this->assertInstanceOf(OptionValidationException::class, $e);
			$this->assertStringContainsString("-Validation failed at: \"foo\" - Only the following values are allowed: \"foo\", \"123\"", $e->getMessage());
		}
	}
